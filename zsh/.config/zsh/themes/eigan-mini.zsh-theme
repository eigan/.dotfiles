...
current_customer() {
    case $PWD/ in
        /srv/cc5/customers/*)
            version=$(realpath $PWD | grep -oP "versions/(\w+)" | sed -e "s/^versions\///")
            customer=$( echo $PWD | grep -oP "cc_(\w+)_(\w+)");
            echo -n $customer
